...
class CustomerTest extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  val FIRST_COLOR_GLOSSY_SECOND_COLOR_MATTE_CUSTOMER = Customer(BitSet(0), Some(1))
  ...
  "Customer" should "be satisfied by all glossy batches" in {
    FIRST_COLOR_GLOSSY_SECOND_COLOR_MATTE_CUSTOMER.isBatchesSatisfactory(ALL_COLORS_GLOSSY_BATCHES) should be (true)
  }

  "Customer" should "be satisfied by two first color matte batches" in {
    FIRST_COLOR_GLOSSY_SECOND_COLOR_MATTE_CUSTOMER.isBatchesSatisfactory(FIRST_TWO_COLORS_MATTE_BATCHES) should be (true)
  }

  "Customer" should "not be satisfied by first color matte batches" in {
    FIRST_COLOR_GLOSSY_SECOND_COLOR_MATTE_CUSTOMER.isBatchesSatisfactory(FIRST_COLOR_MATTE_BATCHES) should be (false)
  }
